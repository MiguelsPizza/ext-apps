 ...
 */
export const McpHttpRequestBodySchema = z
  .union([
    z.object({
      bodyType: z.union([z.literal("none"), z.undefined()]).optional(),
    ...
    z.object({
      ...
      body: z.array(McpHttpFormFieldSchema),
    }),
  ])
  .describe(
    "Discriminated union for type-safe HTTP request body construction.\nUse these types when you want compile-time enforcement of body/bodyType correlation.",
  );
 ...
 * Use these types when you want compile-time enforcement of body/bodyType correlation.
 */
export const McpHttpResponseBodySchema = z
  .union([
    z.object({
      bodyType: z.union([z.literal("none"), z.undefined()]).optional(),
      body: z.undefined().optional(),
  ...
  .describe(
    "Discriminated union for type-safe HTTP response body handling.\nUse these types when you want compile-time enforcement of body/bodyType correlation.",
  );

/**
 * @description Base request fields without body (for use with discriminated union).
 */
export const McpHttpRequestBaseSchema = z.object({
  /** @description HTTP method. Defaults to "GET". */
 ...
 */
export const McpHttpResponseBaseSchema = z.object({
  /** @description HTTP status code. */
  status: z.number().describe("HTTP status code."),
 ...
 */
export const McpHttpRequestStrictSchema = McpHttpRequestBaseSchema.and(
  McpHttpRequestBodySchema,
).describe(
  "Type-safe HTTP request with enforced body/bodyType correlation.\nPrefer this over McpHttpRequest when constructing requests programmatically.",
);
 ...
 */
export const McpHttpResponseStrictSchema = McpHttpResponseBaseSchema.and(
  McpHttpResponseBodySchema,
).describe(
  "Type-safe HTTP response with enforced body/bodyType correlation.\nPrefer this over McpHttpResponse when handling responses programmatically.",
);
